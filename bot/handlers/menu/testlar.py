...
from bot.utils.callbacks.call_data import CategoryData, SubcategoryData
...
async def handler_category(call: CallbackQuery, callback_data: CategoryData):
...
@test_router.callback_query(SubcategoryData.filter())
async def handler_subcategory(call: CallbackQuery, callback_data: SubcategoryData):
    ...
    if quizzes:
        await call.message.edit_text(f"Test {len(quizzes)} ta savoldan iborat",
                                     reply_markup=builder_quizzes_state())
    else:
